...
<section class="hero-bg relative min-h-screen flex flex-col items-center justify-center px-6 py-24 text-white">
  ...
  <!-- Back button -->
  <a href="index.php" id="backBtn" style="position: fixed; top: 24px; left: 24px; z-index: 9999; background: rgba(0,0,0,0); color: white; padding: 12px 24px; border-radius: 12px; text-decoration: none; font-size: 0.95rem; font-weight: 500; transition: all 0.3s;" onmouseover="this.style.color='#60a5fa'" onmouseout="updateBackBtnColor()">
        ...
        <div class="grid md:grid-cols-2 gap-10">
          ...
          <div class="space-y-6">
            <div>
              ...
              <div class="space-y-2">
                ...
                <div class="flex items-center gap-3">
                  <div class="w-2 h-2 rounded-full flex-shrink-0" style="background: var(--amber);"></div>
                  <p class="serif text-xl" style="color: var(--deep-navy);">Leave System</p>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div class="mt-6 p-5 rounded-2xl" style="background: white; border: 1px solid rgba(0,0,0,0.06);">
              ...
              <p class="text-sm leading-relaxed" style="color: var(--slate);">Four web-based systems were developed to digitize and streamline core operations of the DepEd Dasmarinas ICT Unit, covering procurement workflows, employee service records, SARO (Special Allotment Release Order) management, and a leave application portal.</p>
            </div>
          </div>
...
<script>
  ...
  // Back button color change based on scroll position
  const backBtn = document.getElementById('backBtn');
  
  function updateBackBtnColor() {
    const scrollY = window.scrollY;
    const heroHeight = document.querySelector('.hero-bg').offsetHeight;
    
    // If in hero section (dark background), use white text
    if (scrollY < heroHeight - 100) {
      backBtn.style.color = 'white';
    } else {
      // If in light sections, use dark text
      backBtn.style.color = '#1e293b';
    }
  }
  
  // Update color on scroll
  window.addEventListener('scroll', updateBackBtnColor);
  // Set initial color
  updateBackBtnColor();
